...
//     (เพราะข้อมูลจริงถูกเก็บและ sync อยู่ที่ระบบ PRM (dltv_risk_2571) อยู่แล้ว) — คำนวณสดใหม่
//     ทุกครั้งที่เข้าหน้า
...
function _rrgPdfHeaderOpts(subtitle2) {
  return {
    ...
    subtitle: 'มูลนิธิการศึกษาทางไกลผ่านดาวเทียม ในพระบรมราชูปถัมภ์ · ปีงบประมาณ พ.ศ. 2571',
    subtitle2: subtitle2 || ''
  };
}
...
async function rrgExportReportPDF() {
  ...
  try {
    ...
    const qf = document.getElementById('rrgQuarterFilter')?.value || '';
    await _runPdfExport(html, 'riskregister-71-report', _rrgPdfHeaderOpts(qf ? ('ไตรมาส ' + qf) : 'ทุกไตรมาส'));
  } catch(err) {
    console.error(err);
...
async function rrgExportSinglePDF() {
  ...
  try {
    ...
    await _runPdfExport(html, 'riskregister-71-' + r.id, _rrgPdfHeaderOpts('แบบรายงานความเสี่ยงรายการ'));
  } catch(err) {
    console.error(err);
    showToast('❌ สร้าง PDF ไม่สำเร็จ: ' + err.message);
  }
...
async function rrgExportProjectPDF(encProj) {
  ...
  try {
    ...
    await _runPdfExport(html, 'riskregister-71-project-' + safeName, _rrgPdfHeaderOpts('รายงานรายโครงการ (แผน + ผลการดำเนินการ)'), 'landscape');
  } catch(err) {
    console.error(err);
    showToast('❌ สร้าง PDF ไม่สำเร็จ: ' + err.message);
  }
...
async function rrgSyncToSheet() {
  ...
  // sync อยู่ที่ระบบ PRM (dltv_risk_2571) อยู่แล้ว ป้องกันข้อมูลซ้ำซ้อนสองที่
  ...
  try {
    ...
    await gasPost('bulkSaveRiskRegister71', { data: manualList });
